...
public class LauncherConfigJSON: Identifiable, JSONElement
{
   ...
   public required init?( _ json: Any? )
   {
      ...
      family = nonEmptyString( json[ "family" ] )
      fullName = nonEmptyString( json[ "full_name" ] )
      launcherConfigDescription = nonEmptyString( json[ "description" ] )
      variant = nonEmptyString( json[ "variant" ] )
      alias = nonEmptyString( json[ "alias" ] )
      minStage = nonNegativeInt( json[ "min_stage" ] )
      maxStage = nonNegativeInt( json[ "max_stage" ] )
      length = nonNegativeDouble( json[ "length" ] )
      diameter = nonNegativeDouble( json[ "diameter" ] )
      maidenFlight = nonEmptyString( json[ "maiden_flight" ] )
      launchMass = nonNegativeInt( json[ "launch_mass" ] )
      leoCapacity = nonNegativeInt( json[ "leo_capacity" ] )
      gtoCapacity = nonNegativeInt( json[ "gto_capacity" ] )
      takeoffThrust = nonNegativeInt( json[ "to_thrust" ] )
      apogee = nonNegativeInt( json[ "apogee" ] )
      vehicleRange = nonNegativeInt( json[ "vehicle_range" ] )
      ...
      manufacturer = AgencyJSON( json[ "manufacturer" ] )
      programs = parseArray( json[ "program" ] )
      reusable = json[ "reusable" ] as? Bool
      totalLaunchCount = nonEmptyString( json[ "total_launch_count" ] )
      launchCost = nonEmptyString( json[ "launch_cost" ] )
   }
}
