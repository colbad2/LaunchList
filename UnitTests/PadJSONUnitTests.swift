...
class PadJSONUnitTests: XCTestCase
{
   ...
   {
      guard let jsonData: Data = padJSON1.data( using: .utf8 ) else { XCTFail( "can't load test data" ); return }
      let pad: PadJSON? = PadJSON( json: parseJSON( data: jsonData ) )
   ...
   {
      guard let jsonData: Data = padJSON2.data( using: .utf8 ) else { XCTFail( "can't load test data" ); return }
      let pad: PadJSON? = PadJSON( json: parseJSON( data: jsonData ) )
