...
class ProgramJSONUnitTests: XCTestCase
{
   ...
   {
      guard let jsonData: Data = programJSON1.data( using: .utf8 ) else { XCTFail( "can't load test data" ); return }
      let program: ProgramJSON? = ProgramJSON( json: parseJSON( data: jsonData ) )
